...
import { useRoadmapByCourseQuery } from '@/lib/query/roadmap.query';
...
export default function RoadmapDetailPage() {
    ...
    const {
        ...
        isLoading,
        error,
    } = useRoadmapByCourseQuery({ courseId: String(courseId) });
                ...
                <main className="max-w-4xl mx-auto px-6 pt-40 pb-20">
                    ...
                    <div className="relative">
                        ...
                        <div className="space-y-16">
                            {roadmapSteps.data &&
                                roadmapSteps?.data.steps.map((step, index) => (
                                    <div
                                        key={step.id}
                                            ...
                                            {step.status && (
                                                <button
                                                    onClick={() => {
                                                        if (!step.module?.id) {
                                                            console.error(
                                                                'Erreur: Aucun module associé à cette étape',
                                                            );
                                                            return;
                                                        }
                                                        router.push(
                                                            `/course?courseId=${courseId}&moduleId=${step.module.id}`,
                                                        );
                                                    }}
                                                    className="flex items-center gap-3 h-12 px-6 bg-slate-900 text-white rounded-xl font-bold text-sm cursor-pointer hover:bg-primary transition-all"
                                                >
